...
class ChromaStore:
    def __init__(self,directory:str|Path,collection:str) -> None:
        """Inicializa o cliente persistente do ChromaDB e obtém a coleção."""
        import chromadb
    ...
    def upsert(self,ids:list[str],documents:list[str],metadatas:list[dict],embeddings:list[list[float]]) -> None:
        """Insere ou atualiza documentos, metadados e embeddings na coleção."""
        self.collection.upsert(ids=ids,documents=documents,metadatas=metadatas,embeddings=embeddings)
    
    def query(self,embedding:list[float],top_k:int=5,where:dict|None=None) -> list[dict]:
        """Consulta os chunks mais semelhantes ao embedding informado."""
